...
class Employee
  ...
  # attr_reader changed from `private`
  protected

  attr_reader :serial_number

  # abbreviated_serial_number kept private
  private
...
=begin
# Discussion:
access to values of instance varialbles of other objects is not possible if they are
designated as private methods, but are accessible as `protected` methods

LS: Private methods cannot be invoked with an explicit caller, even inside of their own class. 
But on line 62, in Employee#==, we do invoke serial_number with an explicit caller (other).
In order to make this work, we can make serial_number a protected method. 
Recall that from outside the class, protected methods work just like private methods. 
From inside the class, however, protected methods are accessible and may be invoked 
